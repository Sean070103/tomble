  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`match:${matchId}`, { config: { private: true } })
      ...
      .subscribe((status, err) => {
        console.log('Chat channel status:', status, err)
        if (status === 'CHANNEL_ERROR') console.error('Chat channel error:', err)
